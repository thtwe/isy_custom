...
class AdvanceExpenseLine(models.Model):
    ...
    product_uom_id = fields.Many2one('uom.uom', string='Unit of Measure', required=True, readonly=True, default=lambda self: self.env['uom.uom'].search([], limit=1, order='id'))
    ...
    def _onchange_product_id(self):
        for rec in self:
            ...
            if rec.advance_line_id.company_id.currency_id != rec.advance_line_id.currency_id:
                amount = rec.advance_line_id.company_id.currency_id._convert(
                         rec.product_id.standard_price,
                         rec.advance_line_id.currency_id,
                         rec.advance_line_id.company_id,
